// icons.js — shared icon library (Lucide-style 24x24 stroke SVG) + custom-upload rendering
...
window.NAV_ICONS = {
  ...
  heart: {
    label: "Heart",
    svgInner: '<path d="M20.8 4.6a5.5 5.5 0 0 0-7.8 0L12 5.6l-1-1a5.5 5.5 0 0 0-7.8 7.8l1 1L12 21l7.8-7.6 1-1a5.5 5.5 0 0 0 0-7.8z"/>'
  },
  star: {
    label: "Star",
    svgInner: '<polygon points="12 2 15.1 8.6 22 9.3 17 14.1 18.2 21 12 17.6 5.8 21 7 14.1 2 9.3 8.9 8.6 12 2"/>'
  },
  grid: {
    label: "Grid",
    svgInner: '<rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/>'
  }
};
...
// renderTabIcon — branches between the bundled SVG library and a custom-uploaded
// image data URI. Tab icon values are stored as "bundled:<key>" or "custom:<dataURI>".
window.renderTabIcon = function (iconValue, sizePx) {
  const size = sizePx || 16;
  if (!iconValue) return window.renderIcon("chart", size);
  if (iconValue.indexOf("custom:") === 0) {
    const src = iconValue.slice(7);
    return `<img class="tab-icon-img" src="${src}" alt="" width="${size}" height="${size}" />`;
  }
  const key = iconValue.indexOf("bundled:") === 0 ? iconValue.slice(8) : iconValue;
  return window.renderIcon(key, size);
};
